...
export default function EventDetail() {
  ...
  const fetchEventData = async (eventName) => {
    const response = await fetch(`http://localhost:5000/events/${eventName}`);
  ...
  useEffect(() => {
    ...
    if (eventType === "technical") {
      ...
      const eventdata = technicalEvent.find((event) => event.id === eventId);
      // console.log(eventdata);
      setEventDetails(eventdata);
    } else if (eventType === "workshop") {
      setEventBg(workshop);
      const eventdata = workshopevents.find((event) => event.id === eventId);
      setEventDetails(eventdata);
    } else {
      setEventBg(cultural3);
      const eventdata = culturalEvent.find((event) => event.id === eventId);
      // console.log(eventdata);
      setEventDetails(eventdata);
    }
  }, [arr]);
            ...
            >
              <Typography variant="h6">
                {eventDetails.description.split("*")?.length > 1
                  ? eventDetails.description.split("*").map((x, i) => {
                      return (
                        <blockquote
                          key={i}
                          style={{
                            borderLeft: "5px solid #ffffff25",
                            paddingLeft: "10px",
                            marginLeft: "10px",
                            marginTop: "10px",
                            marginBottom: "10px",
                  ...
                  <>
                    ...
                    <div className="sponsorslis">
                      <div className="sponsors">
                        ...
                        <div className="list">
                          {eventDetails.sponsors.gold.map((sponsor, i) => (
                      ...
                      <div className="sponsors">
                        ...
                        <div className="list">
                          {eventDetails.sponsors.silver.map((sponsor, i) => (
                            <EventDetailSponsorCard
                              name={""}
                              logo={sponsor.logo}
                              link={sponsor.link}
          ...
          {eventDetails.eventType !== "workshop" && (
            <CustomTabPanel value={value} index={2}>
              ...
              >
                ...
                <Typography variant="h6">{eventDetails.prizes.pool}</Typography>
                <Typography variant="h6" sx={{ fontWeight: "bold" }}>
              ...
              {eventDetails.phases.map((phase, i) => (
                <Typography variant="h6" key={i}>
                  ...
                  {Object.entries(phase).map((x) => {
                    return (
                      <div key={x[0]}>
                        <h3>{x[0][0].toUpperCase() + x[0].slice(1)}:</h3>
                        <pre
                          style={{
                            whiteSpace: "pre-wrap",
                            wordWrap: "break-word",
